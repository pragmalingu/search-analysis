...
class ComparisonTool(ComparisonToolBase):
        ...
        for obj in eval_objs:
            for con in conditions:
                for query in queries:
                    ...
                    vis_dict["Value"].append(
                        getattr(obj, con)["Query_" + str(query)][con]
                    )
        ...
        for obj in eval_objs:
            ...
            for field in fields:
                for function in explain[field]["details"]:
                    ...
                    explain_dict["Terms"].append(
                        self._extract_terms(function["function"]["description"])
                    )
            ...
            for field in fields:
                for function in explain[field]["details"]:
                    term_dict[obj.name][
                        ...
                        + ": "
                        + (self._extract_terms(function["function"]["description"]))
                    ] = str(function["function"]["value"]).replace(
                        ".", decimal_separator
                    )
    ...
    def calculate_difference(self, condition="fscore", dumps=False):
        ...
        for query, data in getattr(self.eval_obj_1, condition).items():
            if not query == "total":
                ...
                diff[query] = {
                    ...
                    str(self.eval_obj_2.name): getattr(self.eval_obj_2, condition)[
                        query
                    ][condition],
                    diff_name: abs(
                        data[condition]
                        - getattr(self.eval_obj_2, condition)[query][condition]
                    ),
                }
        ...
        diff_ordered["total"] = {
            ...
            diff_name: abs(
                getattr(self.eval_obj_1, condition)["total"]
                - getattr(self.eval_obj_2, condition)["total"]
            ),
        }
        if dumps:
    ...
    def get_disjoint_sets(self, distribution, highest=False):
        ...
        for query, data in getattr(self.eval_obj_1, distribution).items():
            ...
            for res_1 in data[distribution]:
                ...
                if not any(
                    res_1["doc"]["id"] in el["doc"].values()
                    for el in getattr(self.eval_obj_2, distribution)[query][
                        distribution
                    ]
                ):
                    results[query][distribution + " " + self.eval_obj_1.name].append(
                        res_1
                    )
            ...
            for res_2 in getattr(self.eval_obj_2, distribution)[query][distribution]:
                ...
                if not any(
                    res_2["doc"]["id"] in el["doc"].values()
                    for el in getattr(self.eval_obj_1, distribution)[query][
                        distribution
                    ]
                ):
                    results[query][distribution + " " + self.eval_obj_2.name].append(
                        res_2
                    )
            results[query]["count"] = len(
                results[query][distribution + " " + self.eval_obj_1.name]
            ) + len(results[query][distribution + " " + self.eval_obj_2.name])
        filtered_results = {
            key: val for key, val in results.items() if val["count"] != 0
        }
        ordered_results = OrderedDict(
            sorted(filtered_results.items(), key=lambda i: i[1]["count"])
        )
        if not highest:
                        ...
                        if hit["doc"]["id"] == doc_id:
                            try:
                                if not comp_dict[str(obj.name)]:
                                    comp_dict["Document " + str(doc_id)] = {
                                        field: hit["doc"][field] for field in fields
                                    }
                                    comp_dict[str(obj.name)]["position"] = hit[
                                        "position"
                                    ]
                                    comp_dict[str(obj.name)]["score"] = hit["score"]
                                    comp_dict[str(obj.name)]["highlight"] = hit[
                                        "highlight"
                                    ]
    ...
    def visualize_distributions(
        ...
        dist_colors = [
            self.pragma_colors[1],
            self.pragma_colors[4],
            self.pragma_colors[5],
        ]
        ...
        ax = sns.countplot(
            x="Approach", hue="Distributions", data=panda_dist, palette=custom_palette
        )
    ...
    def visualize_explanation_csv(
        ...
        if not eval_objs:
            eval_objs = [self.eval_obj_1, self.eval_obj_2]
        panda_explain = self._get_csv_terms(
            query_id, doc_id, fields, decimal_separator, eval_objs
        )
